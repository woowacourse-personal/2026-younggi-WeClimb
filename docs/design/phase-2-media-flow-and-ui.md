...
현재 `TrimRequest`는 파일 경로를 입력으로 받지만, Phase 1 성공 영상은 MediaStore
URI만 남긴다. 이번 구현에서는 Content URI를 Media3 입력으로 사용하는 경계 또는
트리밍 전용 앱 캐시 원본 보존 중 하나를 확정해야 한다. 이 선택은 구현 시작 전 작은
기술 스파이크로 결정한다.
...
UI는 큰 단일 `MainActivity` 화면 코드를 화면별 Compose와 공통 디자인 토큰으로
나눈다. 기능이 아직 없는 종료, 리포트, 기록 화면은 route와 화면 구현을 추가하지
않는다.
...
- [ ] #1 트리밍 가능한 성공 영상의 영속 모델과 상태 전이를 설계하고 shared 테스트를
  작성한다.
- [ ] #2 MediaStore URI와 Media3 Transformer를 연결하는 Android 트리밍 경계를
  스파이크와 테스트로 검증한다.
- [ ] #3 Room entity, DAO, repository에 트리밍 상태와 아카이브 조회를 추가한다.
- [ ] #4 즉시 트리밍, 나중에 처리, 원본 유지 흐름을 세션 보드와 트리밍 화면에
  연결한다.
- [ ] #5 아카이브 목록, 영상 재생, 삭제된 영상 플레이스홀더를 구현한다.
- [ ] #6 선택 영상의 `ACTION_SEND` 공유를 아카이브와 트리밍 화면에 연결한다.
- [ ] #7 디자인 토큰과 공통 Compose 컴포넌트를 만들고 현재 실제 화면에
  design-bundle UI를 적용한다.
- [ ] #8 단위, Room 통합, Android Intent 테스트와 실기기 트리밍, 재생, 공유
  walkthrough를 완료한다.

## 리스크 및 미해결 질문

- 트리밍 입력으로 Content URI를 직접 사용할지, 앱 캐시 원본을 보존할지 추가 논의
  필요
- 트리밍 출력이 원본을 교체할지, 원본과 결과를 함께 보존할지 미정
- 트리밍 대기열의 세션 종료 후 노출 위치는 아카이브를 기본으로 하되 세부 UX는 구현
  중 확정
- 아카이브 영상 썸네일 생성 방식과 대용량 목록 성능 정책은 미정
- 기존 `03-tech-decisions.md`의 완료 전 Phase 1 단계와 handoff의 예전 Override
  명칭은 별도 문서 정합성 작업으로 갱신 필요
