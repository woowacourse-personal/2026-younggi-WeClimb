...
- 녹화 뒤 분류 전 이탈 시 미분류 Attempt 보관과 아카이브 재분류
- 저장 대기 영상이 있는 세션 종료 보호와 실제 종료 요약
- 나중 트리밍 대기열 보존과 재진입
- 자르지 않음을 선택하면 원본 유지
...
- 서버 동기화, 계정 기능, iOS 구현
- `09-session-end`, `10-report`, `11-records`의 정적 UI를 넘어선 제품 행동
...
- 트리밍 실패 또는 범위 오류는 원본과 시도 기록을 보존하고 사용자가 재시도하거나
  원본을 유지할 수 있게 한다.
- 아카이브는 성공 영상과 미분류 시도를 최신순으로 보여 주고, 세션 날짜, 암장명,
  색상, 분류 또는 트리밍 상태를 표시한다.
...
- [x] #7 기존 bundle의 토큰 방향 적용은 완료했다. 새 bundle을 받은 뒤
  [UI 재구현 계약](ui-rebuild-contract.md)에 따라 시안 충실도 Compose UI를
  재구현한다.
- [x] #8 단위, Room 통합, Android Intent 테스트와 실기기 트리밍, 재생, 공유
  walkthrough를 완료한다.
- [x] #9 미분류, 저장 대기 보호, 실제 종료 요약의 변경 중심 실기기 테스트를
  완료한다.
- [ ] #10 시도 결과와 영상 보관 분리, 실제 데이터 연결과 새 디자인 작업 뒤 남은
  UI scenario를 다시 감사하고 전체 회귀와 traceability를 완료한다.
...
- 트리밍을 앱 종료 뒤에도 보장하는 백그라운드 처리 방식은 후속 기술 범위다.
  현재는 중단 뒤 원본과 재시도 가능한 상태를 복구한다.
- 미분류 카드와 저장 대기 종료 상태의 전용 시안은 다음 design-bundle에서 확정한다.
