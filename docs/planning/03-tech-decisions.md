...
**다음 스텝 (순서대로)**:
1. **스파이크 (1~2일)** — 위 체크리스트 5개를 실기기에서. 특히 ②(edit-list
   트림 결과물이 인스타 업로드 후에도 정확한지)가 마지막 불확실성
2. **design-doc** — Phase 1 범위 기술 설계 (KMP 모듈 구조 · 데이터 모델 ·
   Supabase 스키마 · repository 경계)
3. **Phase 1 구현 (1주차)** — 온보딩·홈·암장선택·세션보드·인앱촬영,
   완료 신호 = "실제 암장에서 앱으로만 찍고 완등이 기록된다"
4. Phase 2 (2주차) — 트리밍·세션종료·리포트·인스타 공유 = MVP
