...
**Phase 2와 UI 재구현 완료 범위**:

- 성공 영상의 즉시 트리밍, 나중에 처리, 원본 유지
- 성공 영상 아카이브, 앱 내 재생, `ACTION_SEND` 공유
- `00-common`부터 `11-records`까지 design-bundle 구조를 Compose로 재구현.
  `09`부터 `11`은 정적 Phase 3 UI이며 제품 행동은 비활성
- 색 행 탭으로 영상 없는 성공 Attempt를 기록하고, 개인 암장 추가 후 선택,
  명시적 세션 종료 확인, 트리밍 완료 화면 유지까지 frozen UI 계약에 연결
- 녹화 뒤 미분류 보관과 아카이브 재분류, 분류 중복 입력 차단, 저장 대기 중
  세션 종료 보류, 실제 전체 시도와 정리 필요 요약을 frozen UI 계약에 연결
- 2026-07-29 전체 instrumentation 23건과 2026-08-04 변경 중심 instrumentation
  6건이 SM-S911N에서 통과. 최신 변경을 포함한 전체 회귀 실행은 보류
- `shared:jvmTest`, Android 단위 테스트, debug APK와 instrumentation 컴파일,
  Android lint 통과. 390dp debug catalog 접근성 표면과 스크린샷 생성 확인

**다음 결정이 필요한 항목**:

- `UNCLASSIFIED` 카드와 재분류, `SAVE_PENDING` 종료 보류, 새로운 종료 요약을
  design-bundle에 어떤 시각 구조로 반영할지
- 기존 Phase 1 설치본의 Room v1 성공 Attempt를 영상 처리 상태가 있는 현재 모델로
  옮길지와 기존 영상 보존 정책
- 세션 종료 사진, 리포트 카드, 기록 통계, 리포트 기반 공유의 제품 계약과 구현 순서
- 트리밍을 앱 종료 뒤에도 완료하는 WorkManager 또는 foreground service 기반
  백그라운드 처리 계약

**다음 스텝 (순서대로)**:
1. 별도 worktree에서 시도 결과와 영상 보관 결정을 분리하는 행동 계약 확정
2. 실제 기능 화면의 시간, 개수, 날짜, 상태와 미디어 메타데이터를 런타임 데이터로 연결
3. 사용자와 별도 Claude Design에서 갱신한 design-bundle 수령
4. 새 시안 기준의 모션과 추가 사용자 플로우 구현 및 검증
5. 기존 S1과 S7-S10을 새 구조에서 다시 감사하고 필요한 항목만 마감
6. 최신 전체 frozen 명령, Scenario Traceability와 문서 마감
7. 기존 설치본의 영상 상태 마이그레이션과 보존 정책 결정
8. 세션 종료 후처리, 리포트 카드, 기록 통계, 리포트 기반 공유의 설계 계약 확정
9. 다음 계약을 구현하고 실제 암장 사용으로 세션 기록 흐름을 관찰
