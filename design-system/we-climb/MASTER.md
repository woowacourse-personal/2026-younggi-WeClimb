...
### B. 홀드 데이터 색 (루트 색깔 — 차트·칩 전용)

다크 배경에서 구분되도록 400번대 톤으로 보정. **색만으로 의미 전달 금지** —
칩·차트에 항상 한글 라벨(하양/노랑/초록/파랑/빨강/보라)을 병기한다.
...
> 홀드 데이터 색은 브랜드 색(오렌지)과 겹치지 않으므로 오렌지가 항상 "액션"
> 시그널로 남는다. 색 순서(난이도)는 암장마다 다르니 하드코딩 금지 — 순서는
> 세션의 암장 메타데이터를 따른다.
...
- **바텀 탭**: 홈 / 기록 2개. Material3 `NavigationBar`, 세이프에어리어 준수.
