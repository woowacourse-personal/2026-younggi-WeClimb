...
## 결과와 영상 보관 (2026-08-05 · Phase 2 Flow v2 핵심)

시도 결과와 영상 보관 여부는 별개다. v1처럼 "성공=보관 / 실패=삭제"로 묶지
않는다. **실패해도 무빙이 좋으면 영상을 남긴다.**

다만 이 둘을 **두 단계로 나누어 묻지는 않는다.** 촬영 직후 화면의 **버튼 3개**가
결과와 보관 여부를 한 번에 정한다.
...
레이아웃은 **큰 두 버튼(실패 · 완등) + 전체 폭 보조 버튼**이다. 세 개를 나란히
두지 않는다 — 세 번째는 대등한 결과가 아니라 **실패의 변형**이고, 같은 줄에 두면
매 시도마다 셋을 읽게 된다.

`AttemptMediaState`는 기존 enum을 그대로 쓴다: `NONE`, `TRIM_PENDING`,
`ORIGINAL_KEPT`, `TRIM_PROCESSING`, `TRIMMED`, `TRIM_FAILED`.

### 촬영 직후에는 영상 처리를 묻지 않는다

보관되는 영상은 **예외 없이 `TRIM_PENDING`** 으로 대기함에 들어가고 바로 보드로
돌아간다. 자르기·원본 유지·공유·지우기는 **전부 `07` 아카이브**에서 한다.
`06` 트리밍의 유일한 진입점도 아카이브다.

암장에서 벽 앞에 서서 트리밍하지 않는다. 답이 거의 항상 "나중에"인 질문을 매 시도
마다 띄우지 않는다.

### 실패 삭제에는 되돌리기 창을 붙인다

`실패`는 탭 한 번으로 영상을 지운다. 매번 확인 다이얼로그를 띄우면 세션이 멈추므로
**5초 되돌리기 창**을 대신 쓴다. 그래서 **캐시 삭제를 창이 끝날 때까지 지연**해야
한다 — 먼저 지우면 되돌릴 것이 없다.

### 아카이브는 결과로 영상을 분류하지 않는다

카드의 분류축은 **미디어 상태**다. 완등·실패는 **색 옆의 작은 칩**으로만 붙고,
필터도 결과가 아니라 정리 상태·암장 기준이다. 영상을 남긴 이유는 결과가 아니라
**영상의 가치**이므로, 무빙이 좋아 남긴 영상이 "실패 영상"으로 분류되면 안 된다.
결과 자체는 복기에 유용한 맥락 정보로 남긴다.
...
> 없다. 따라서 **기능 표면(02 홈 실데이터 영역, 04 보드, 05 촬영, 05b 저장 대기,
...
- **바텀시트** (`ModalBottomSheet`): 저장 대기 해소, 미분류 재분류, 카운트 조정,
  개인 암장 추가, 암장 관리 메뉴에 사용. 상단 grab 핸들, radius 24dp,
...
- **3버튼 분류**: `실패` · `완등` 큰 두 버튼 + `실패지만 영상은 남기기` 전체 폭
  보조 버튼. 영상 결과를 **버튼 부제에 명시**한다 — 탭 한 번으로 영상이 지워지므로
  사전 고지가 필수다.
- **되돌리기 칩 / 배너**: 보드 오탭(+1)과 촬영 실패(영상 삭제)를 5초 안에 회수한다.
  진행 막대로 남은 시간을 보이되 숫자는 쓰지 않는다. TalkBack 활성 시 창을 연장한다.
- **카운트 조정 시트**: 색 행 롱프레스(500ms) → 스테퍼(− 값 +). `−`는 **영상 없는
  최근 시도**부터 지운다. 영상 있는 시도만 남으면 `−`를 잠그고 아카이브로 보낸다 —
  오탭 정정이 영상 삭제로 번지면 안 된다.
- **저장 대기 해소 시트**: `SAVE_PENDING`의 `다시 저장` / `영상만 폐기하고 기록 유지`.
- **미분류 재분류 시트**: 아카이브 `분류 필요` 카드에서 05와 **동일한 3버튼**으로
  결과를 확정한다.
...
### 세션 보드 진행 막대 — 바퀴 덧칠 (v1 규칙 유지)

`LAP = 5완등`. `lap = count / 5`, `rem = count % 5`.

- `lap == 0` → 바탕만 `rem/5 × 100%`, shade 0. 덧칠 없음
- `lap >= 1` → 바탕 **100%** shade `lap-1` + 덧칠 `rem/5 × 100%` shade `lap`

예: `2`→40% shade0 · `5`→100% shade0 · `7`→100% shade0 + 40% shade1 ·
`12`→100% shade1 + 40% shade2.

막대는 **절대량**을 나타낸다. "최다 색 대비 비율"로 그리면 다른 색을 올릴 때마다
내 막대가 줄어들어, 기록은 늘었는데 시각적으로 후퇴한다.

바퀴 shade는 **`RGB × 0.70`** 으로 만든다. 표준 색 램프(lap 0~3):

| 색 | lap 0 | lap 1 | lap 2 | lap 3 |
|---|---|---|---|---|
| 하양 | `#E2E8F0` | `#9EA2A8` | `#6F7276` | `#4E5052` |
| 노랑 | `#DDBB44` | `#9B8330` | `#6C5C21` | `#4C4017` |
| 초록 | `#46AE72` | `#2E7A52` | `#20573A` | `#183C27` |
| 파랑 | `#5A8FDB` | `#3E6CB5` | `#2C466B` | `#1F314B` |
| 빨강 | `#E45D5D` | `#A04141` | `#702E2E` | `#4E2020` |
| 보라 | `#A78BFA` | `#7561AF` | `#52447A` | `#393056` |
| 주황 | `#E68A4C` | `#A16135` | `#714425` | `#4F2F1A` |
| 남색 | `#7C7FDB` | `#575999` | `#3D3E6B` | `#2B2C4B` |
| 갈색 | `#AC7B52` | `#785639` | `#543C28` | `#3B2A1C` |
| 검정 | `#1E293B` | `#334155` | `#475569` | `#64748B` |

초록·파랑 lap1~2는 v1 승인값을 그대로 유지했다(규칙값과 채널당 3~5 차이, 육안 동일).

**검정은 예외** — 배경 `#0B0F1A`와 이미 가까워 더 어둡게 하면 막대가 사라진다.
slate 램프로 **밝아지는** 방향으로 진행한다. 암장 커스텀 색도 `RGB × 0.70` 결과가
배경 대비 1.5:1 미만이면 같은 방식으로 전환한다.

바퀴 수를 **색으로만 읽게 하지 않는다** — 숫자 카운트가 정본이고 덧칠은 보조 신호다.
...
- 촬영 직후에는 영상 처리를 묻지 않는다. 보관 영상은 전부 `TRIM_PENDING`이고,
  자르기·원본 유지·공유·지우기는 `07` 아카이브에만 둔다.
- 되돌릴 수 있는 삭제(촬영 실패)는 **확인 대신 되돌리기 창**을 쓴다. 대신 실제
  삭제를 창 종료까지 지연한다. 되돌릴 수 없는 삭제(아카이브)는 확인을 거친다.
- 세션 종료 요약은 완등 · 전체 시도 · **저장한 영상** 세 값이다. 촬영 영상이 전부
  대기함으로 가므로 "정리 필요"는 정보량이 없다.
...
- [ ] 분류가 3버튼이고 영상 결과가 버튼 부제에 명시됨
- [ ] `FAILURE` + `TRIM_PENDING`(실패지만 영상 보관) 경로가 UI에 존재
- [ ] 촬영 직후 영상 처리 시트가 **없음**. 보관 영상은 전부 `TRIM_PENDING`
- [ ] 실패 캐시 삭제가 되돌리기 창 종료까지 **지연**됨
- [ ] 보드 오탭 되돌리기(칩)와 롱프레스 조정(스테퍼)이 동작
- [ ] 조정 시트에서 **영상 있는 시도의 − 가 잠김**
- [ ] 아카이브 카드가 결과가 아니라 **미디어 상태**로 분류됨
