...
| Deferred / Inert | `#C084FC` | **Phase 3 · 비활성** 뱃지 — 구현됐으나 아직 동작 안 하는 UI 표시. 배경 `rgba(168,119,210,.14)`, 테두리 `rgba(168,119,210,.40)`. 브랜드 오렌지·레벨색과 충돌 안 하도록 보라 계열로 분리 |
...
- **아카이브 카드 상태 pill**: 트리밍 완료(success), 트리밍 대기(brand), 원본
  ...
  구분 금지 — 라벨 병기.
- **Phase 3 · 비활성 뱃지**: Deferred/Inert 색(`#C084FC`) 사용. 구현됐으나 미작동
  UI(리포트 공유, 통계, 세션 종료 사진, 홈 스냅샷)에 부착. 실제 구현 시 이 뱃지가
  붙은 요소는 시각만 완성하고 동작은 비활성/후속.
